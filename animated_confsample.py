...
# Creates animated GIF of sampling of energetically favorable conformers,
# optionally within binding site
...
parser = argparse.ArgumentParser(description="Generates gif of conformational \
sampling of input small molecule, optionally in a binding site \
(prealignment assumed).")
parser.add_argument('-l', '--ligand', help='Small molecule whose conformers \
will be sampled')
parser.add_argument('-r', '--receptor', required=False, help='Optionally \
...
if ext == "sdf":
    mol = Chem.MolFromMolFile(args.ligand)
elif ext == "pdf":
    mol = Chem.MolFromPDBFile(args.ligand)
else:
    print "Unknown ligand extension"
    sys.exit()
