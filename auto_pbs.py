...
run_script.write('#PBS -q dept_gpu\n\n')
...
run_script.write("SCRDIR=/scr/$PBS_JOBID\n\n")
run_script.write("if [[ ! -e $SCRDIR ]]; then\n")
run_script.write("\tmkdir $SCRDIR\n")
run_script.write("fi\n\n")
run_script.write("echo scratch drive ${SCRDIR}\n\n")
run_script.write("cp $PBS_O_WORKDIR/*.in ${SCRDIR}\n")
run_script.write("cp $PBS_O_WORKDIR/*.prmtop ${SCRDIR}\n")
run_script.write("cp $PBS_O_WORKDIR/*_md2.rst ${SCRDIR}\n")
run_script.write("cp $PBS_O_WORKDIR/*.inpcrd ${SCRDIR}\n\n")
run_script.write("cd ${SCRDIR}\n\n")
run_script.write("export AMBERHOME=/usr/local/amber14\n")
run_script.write("export PATH=/usr/local/amber14/bin:$PATH\n")
run_script.write("export \
LD_LIBRARY_PATH=/usr/local/cuda-6.5/lib64:/usr/local/amber14/lib:/usr/include/x86_64-linux-gnu:$LD_LIBRARY_PATH\n\n")
